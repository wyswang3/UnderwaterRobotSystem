...
- `UnderWaterRobotGCS/src/urogcs/telemetry/ros2_mirror_adapter.py`
- `UnderWaterRobotGCS/src/urogcs/telemetry/ros2_mirror_source.py`
...
- TUI 是成熟主路径
- PySide6 GUI 已有首页骨架和 read-only ROS2 preview source
- ROS2 bridge 当前只提供只读 mirror topic 和 advisory 派生 topic
...
- `/rov/health_monitor`
  - advisory `HealthMonitorStatus` 派生 topic
...
4. `HealthMonitorStatus` 只能提供外围摘要和恢复建议，不是新的安全 authority。
5. bridge topic 只用于 UI backend / diagnostics / rosbag / tools，不允许回灌控制或导航主链。

## 8. GCS ROS2 preview 约束

当前 GUI 已支持 `telemetry_source=ros2` 的 preview 入口。

它的语义边界是：

1. 只消费 `/rov/telemetry` mirror，不直接发送控制命令。
2. 复用现有 `StatusTelemetry` 压缩语义和 `TelemetrySnapshot` / `ui_viewmodels`。
3. `session_established` / `link_alive` 在没有 gateway session topic 时，只能从 `system.session_state` 做保守推断。
4. 这条 preview 路径不能替代当前 UDP teleop 主路径。
